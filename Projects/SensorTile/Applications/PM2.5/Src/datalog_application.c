...
uint8_t DATALOG_SD_Log_Enable(void)
{
  ...
  char header[] = "Timestamp\tAccX [mg]\tAccY [mg]\tAccZ [mg]\tGyroX [mdps]\tGyroY [mdps]\tGyroZ [mdps]\tMagX [mgauss]\tMagY [mgauss]\tMagZ [mgauss]\tP [mB]\tPM2.5 \t\r\n";
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
      ...
      if ( verbose == 1 )
      {
        ...
        {
          printf(  "WHO AM I address[%d]: ERROR\n", id );
        }
        else
        {
          printf( "WHO AM I address[%d]: 0x%02X\n", id, who_am_i );
        }
 ...
 void PM(void)
{
	 if(SendOverUSB)
	 {
		 ...
		 if(pmbuff[3]<=0x04) 				printf("\r\npm2.5:%d.%02d,空气状况：优",pmbuff[3],pmbuff[4]);	

	else if (pmbuff[3]<=0x08)   printf("\r\npm2.5:%d.%02d,空气状况：良",pmbuff[3],pmbuff[4]);
 	
	
	else if (pmbuff[3]<=0x0c)		printf("\r\npm2.5:%d.%02d,空气状况：中",pmbuff[3],pmbuff[4]);
	
	else 												printf("\r\npm2.5:%d.%02d,空气状况：差",pmbuff[3],pmbuff[4]);
    ...
    {
      uint8_t size;
      size = sprintf( dataOut,"%d.%02d\t", (int)(pmbuff[3]),(int)(pmbuff[4]));
      res = f_write(&MyFile, dataOut, size, (void *)&byteswritten);
    }
